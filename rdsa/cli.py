...
def process_raw(raw_posts, source, args, c):
    ...
    inv=load_inventory(config.INVENTORY_CSV)
    if pilot:
        real, _report=load_real_inventory(config.INVENTORY_REAL_CSV)
        if real: inv=real
    ...
    for item in raw_posts:
        ...
        if lead.lead_class in ('hot_lead','qualified_lead'):
            print(format_preview_card(lead) if pilot else format_card(lead)); print()
...
def run_pilot_scan(args):
    if not apify_live_enabled():
        ...
        return {'new': 0, 'alerts': 0, 'duplicates': 0, 'classes': {}}
    c=connect(config.DB_PATH)
    raw=ApifyThreadsProvider().search_batched(config.PILOT_QUERIES, max_posts_per_query=5,
    ...
    print(f"Pilot scan complete: {result['new']} new leads, {result['duplicates']} duplicates (preview only)")
def main(argv=None):
