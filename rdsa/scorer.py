...
from .scoring_config import SCORE_VERSION
...
def score(lead, now=None):
    ...
    add("R3",15 if lead.budget_max is not None else 7 if re.search(r"murah|under|flexible budget",t) else 0,"budget stated")
    ...
    add("R6",12 if re.search(r"secepatnya|asap|bulan ini|akhir bulan",t) else 6 if re.search(r"bulan depan|next month",t) else 2 if re.search(r"tahun depan|someday",t) else 0,"move-in urgency")
    ...
    if re.search(r"giveaway|join sekarang|modal .* jadi|klik link|follow \+ repost",t): add("P2",-40,"spam signal")
